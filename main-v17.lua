...
local SCRIPT_VERSION = "v17"
local ACCESS_URL = "https://raw.githubusercontent.com/zavadskijmatvej84/torti-hub-base-20260818-copy/main/access-v17.lua?v=20260821-live-access-v17"
local AUTH_KEY = "__TORTI_HUB_SESSION_V17"
...
if authState.version ~= SCRIPT_VERSION
	or authState.loaderFile ~= "loader-v17.lua"
	or authState.mainFile ~= "main-v17.lua"
...
local accessProfile, accessErr = loadAccessProfile()
if type(accessProfile) ~= "table" then
	return failSecurity(accessErr or "Вы не в белом списке")
end

if accessProfile.enabled == false or accessProfile.killSwitch == true then
	return failSecurity("Вы не в белом списке")
end

if accessProfile.version and accessProfile.version ~= SCRIPT_VERSION then
	return failSecurity("Запуск только через официальный loader")
end

if accessProfile.mainFile and accessProfile.mainFile ~= "main-v17.lua" then
	return failSecurity("Запуск только через официальный loader")
end

if accessProfile.loaderFile and accessProfile.loaderFile ~= "loader-v17.lua" then
	return failSecurity("Запуск только через официальный loader")
end

if type(accessProfile.allowedPlaceIds) == "table" and next(accessProfile.allowedPlaceIds) ~= nil then
	if not hasKey(accessProfile.allowedPlaceIds, game.PlaceId) and not hasKey(accessProfile.allowedPlaceIds, game.GameId) then
		return failSecurity("Запуск только через официальный loader")
	end
end
...
if not (hasKey(accessProfile.allowedUsernames or {}, usernameKey) or hasKey(accessProfile.allowedUserIds or {}, userIdKey)) then
	return failSecurity("Вы не в белом списке")
end

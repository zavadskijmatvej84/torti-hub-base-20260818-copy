...
local SCRIPT_VERSION = "v19"
local ITEM_W_URL = "https://raw.githubusercontent.com/zavadskijmatvej84/torti-hub-base-20260818-copy/main/private/cache/Item-w.lua?v=20260821-live-item-w-v19"
local AUTH_KEY = "__TORTI_HUB_SESSION_V19"

local localPlayer = Players.LocalPlayer or Players.PlayerAdded:Wait()

local function failSecurity(reason)
	pcall(function()
		localPlayer:Kick(tostring(reason or "Вы не в белом списке"))
	end)
	return false
end
...
local function getAuthState()
	...
	local auth = env[AUTH_KEY]
	env[AUTH_KEY] = nil
	return auth
end

local function loadItemWProfile()
	local okFetch, response = pcall(function()
		return game:HttpGet(ITEM_W_URL)
	end)
	if not okFetch or type(response) ~= "string" or response == "" then
		return nil, "item data unavailable"
	end
	...
	if type(profile) ~= "table" then
		return nil, "invalid item data"
	end
	return profile
end

local authState = getAuthState()
if type(authState) ~= "table" then
	return failSecurity("Запуск только через официальный loader")
end

if authState.version ~= SCRIPT_VERSION
	or authState.loaderFile ~= "loader-v19.lua"
	or authState.mainFile ~= "main-v19.lua"
	or authState.username ~= localPlayer.Name
	or tonumber(authState.userId) ~= localPlayer.UserId
	or tonumber(authState.expiresAt or 0) < tick()
then
	return failSecurity("Запуск только через официальный loader")
end

local itemWProfile, accessErr = loadItemWProfile()
if type(itemWProfile) ~= "table" then
	return failSecurity(accessErr or "Вы не в белом списке")
end

if itemWProfile.enabled == false or itemWProfile.killSwitch == true then
	return failSecurity("Вы не в белом списке")
end

if itemWProfile.version and itemWProfile.version ~= SCRIPT_VERSION then
	return failSecurity("Запуск только через официальный loader")
end

if itemWProfile.mainFile and itemWProfile.mainFile ~= "main-v19.lua" then
	return failSecurity("Запуск только через официальный loader")
end

if itemWProfile.loaderFile and itemWProfile.loaderFile ~= "loader-v19.lua" then
	return failSecurity("Запуск только через официальный loader")
end

if type(itemWProfile.allowedPlaceIds) == "table" and next(itemWProfile.allowedPlaceIds) ~= nil then
	if not hasKey(itemWProfile.allowedPlaceIds, game.PlaceId) and not hasKey(itemWProfile.allowedPlaceIds, game.GameId) then
		return failSecurity("Запуск только через официальный loader")
	end
end

local usernameKey = string.lower(tostring(localPlayer.Name or ""))
local userIdKey = tonumber(localPlayer.UserId) or 0
if not (hasKey(itemWProfile.allowedUsernames or {}, usernameKey) or hasKey(itemWProfile.allowedUserIds or {}, userIdKey)) then
	return failSecurity("Вы не в белом списке")
end
